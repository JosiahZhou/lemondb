...
bool Database::testExist(const std::string &tableName) {
    auto it = this->tables.find(tableName);
    if (it != this->tables.end()) {
        return true;
    }
...
Table &Database::copyTable(std::string table, std::string newtable){
    auto &db = Database::getInstance();
    std::stringstream is;
    is << db[table];
    ...
    std::string errString =
            !table.empty() ?
            R"(Invalid table (from "?") format: )"_f % table :
    ...
    auto t = std::make_unique<Table>(tableName, fields);
    ...
    while (std::getline(is, line)) {
        ...
        t->insertByIndex(key, std::move(tuple));
    }

    return db.registerTable(std::move(t));
}
